...
impl Rand for MTRng64 {
    fn rand<R: Rng>(rng: &mut R) -> MTRng64 {
        MTRng64::new(rng.gen())
    }
}
...
mod test {
    ...
    #[test]
    fn test_vector() {
        let mut rng: MTRng64 = SeedableRng::from_seed(&[0x12345u64, 0x23456, 0x34567, 0x45678] as &[u64]);
        let values: Vec<u64> = rng.gen_iter().take(TEST_VECTOR.len()).collect();

        assert_eq!(values, &TEST_VECTOR as &[u64]);
    }
}
